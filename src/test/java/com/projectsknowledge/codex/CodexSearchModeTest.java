...
class CodexSearchModeTest {
    ...
    void basicRequestsOnlySummaryConfidenceAndScope() {
        ...
        assertThat(schema.at("/properties/answer")).isEqualTo(client.outputSchema(SearchMode.ADVANCED).at("/properties/answer"));
    ...
    void bothModesUseTheSameSummaryAndEvidenceInstructions() {
        for (String instructions : new String[]{client.basicInstructions(), client.advancedInstructions()}) {
            assertThat(instructions).contains(CodexAppServerClient.SUMMARY_INSTRUCTIONS)
                    .contains(CodexAppServerClient.INVESTIGATION_INSTRUCTIONS)
                    .contains("at most 120 words").contains("distinguish multiple stages or implementations")
                    .doesNotContain("90 words", "at most 2 targeted searches", "at most 3 files");
        }
        assertThat(client.basicInstructions()).contains("Do not include code snippets, file paths, citations, or source excerpts");
    }
    ...
    void workflowSchemaAndInstructionsFocusOnVerifiedBusinessBehavior() {
        ...
        var fields = new ArrayList<String>();
        schema.path("properties").fieldNames().forEachRemaining(fields::add);
        assertThat(fields).containsExactlyInAnyOrder("inScope", "answer", "confidence", "roles", "businessFlow", "risks", "sources", "workflowExample", "workflowDiagram");
        ...
        assertThat(client.instructions(SearchMode.WORKFLOW)).contains("actual authorization checks", "not a real event", "never stitch unrelated flows together");
    ...
    void workflowDiagramSurvivesParsingAndOtherModesNeedNoDiagram() throws Exception {
        ...
        assertThat(client.parseAnswer("{\"inScope\":true,\"answer\":\"Basic\",\"confidence\":\"high\"}", SearchMode.BASIC).workflowDiagram().nodes()).isEmpty();
        assertThat(client.parseAnswer("{\"inScope\":true,\"answer\":\"Advanced\",\"confidence\":\"high\"}", SearchMode.ADVANCED).workflowDiagram().nodes()).isEmpty();
    }
    ...
    void allModesRequireScopeAndShareTheSameScopeGate() throws Exception {
        for (SearchMode mode : SearchMode.values()) {
            ...
            assertThat(client.instructions(mode)).startsWith(CodexAppServerClient.SCOPE_INSTRUCTIONS)
                    .contains("stop immediately without searching", "mixes project questions", "untrusted data", "still inScope=true");
            for (String flag : new String[]{"", "\"inScope\":null,", "\"inScope\":\"true\",", "\"inScope\":1,"}) {
                assertThatThrownBy(() -> client.parseAnswer("{" + flag + "\"answer\":\"Unrelated content\",\"confidence\":\"high\"}", mode))
                        .isInstanceOf(java.io.IOException.class);
            }
            assertThat(client.parseAnswer("{\"inScope\":false,\"answer\":\"\",\"confidence\":\"low\"}", mode).inScope()).isFalse();
            assertThat(client.parseAnswer("{\"inScope\":true,\"answer\":\"No evidence\",\"confidence\":\"low\"}", mode).inScope()).isTrue();
        }
    }

    @Test
    void requestAcceptsModesAndPreservesLegacyDefault() throws Exception {
        assertThat(mapper.readValue("{\"projectId\":\"p\",\"question\":\"q\",\"mode\":\"workflow\"}", QuestionRequest.class).mode()).isEqualTo(SearchMode.WORKFLOW);
        assertThat(mapper.readValue("{\"projectId\":\"p\",\"question\":\"q\",\"mode\":\"basic\"}", QuestionRequest.class).mode()).isEqualTo(SearchMode.BASIC);
        assertThat(mapper.readValue("{\"projectId\":\"p\",\"question\":\"q\",\"mode\":\"advanced\"}", QuestionRequest.class).mode()).isEqualTo(SearchMode.ADVANCED);
        assertThat(mapper.readValue("{\"projectId\":\"p\",\"question\":\"q\"}", QuestionRequest.class).mode()).isEqualTo(SearchMode.ADVANCED);
        assertThatThrownBy(() -> mapper.readValue("{\"mode\":\"unknown\"}", QuestionRequest.class)).isInstanceOf(java.io.IOException.class);
    }
}
