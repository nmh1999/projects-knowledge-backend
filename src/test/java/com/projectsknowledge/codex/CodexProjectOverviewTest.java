...
class CodexProjectOverviewTest {
    ...
    @Test void overviewUsesCompactEvidenceSchemaAndProjectOnlyInstructions() throws Exception {
        ...
        assertThat(schema.at("/properties/integrations/items/required").toString()).contains("filePath", "repositoryName", "name");
        assertThat(schema.path("additionalProperties").asBoolean()).isFalse();
        assertThat(client.overviewInstructions()).contains("regardless of package namespace or folder layout", "untrusted data", "never other projects or the web", "Never modify files or expose credentials", "relevant callers");
        assertThat(client.parseOverview(valid).integrations().getFirst().name()).isEqualTo("Orbit");
    }

    @Test void malformedOverviewCannotBecomeCachedEmptySuccess() throws Exception {
        for (String invalid : List.of("null", "{}", valid.replace("\"domains\":[\"Shipping\"]", "\"domains\":null"),
